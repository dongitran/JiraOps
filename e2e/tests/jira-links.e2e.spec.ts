...
test.describe('Jira Ops assigned ticket workflow', () => {
  test('User can review JiraOps 0.1.23 stable release notes', async () => {
    ...
    try {
      ...
      await expect(whatsNewFrame.getByText('JiraOps 0.1.23 Stable')).toBeVisible();
      await expect(whatsNewFrame.getByLabel('Release highlights')).toContainText(
        'Stable Release'
      );
      await expect(whatsNewFrame.getByText('0.1.22')).toHaveCount(0);
    } finally {
